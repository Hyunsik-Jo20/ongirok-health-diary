...
function filterAlreadyProvidedMissingData(items = [], payload = buildPayload()) {
  ...
  const hasWorkoutDetail = narrativeHas(combined, /근력|웨이트|세트|횟수|회|분|시간|km|심박|bpm|페이스|케이던스|RPE|체감\s*강도/i);
  return (Array.isArray(items) ? items : []).filter(item => {
    const text = String(item || "");
    if (record.waterLiters && /수분|물\s*섭취|음수/.test(text)) return false;
    if (record.steps && /걸음|보행|활동량/.test(text)) return false;
    if (record.sleepHours && /수면\s*(시간|량)|총\s*수면|취침.*기상/.test(text)) return false;
