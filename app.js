...
9. dailyRecord의 steps, sleepHours, waterLiters, narrative는 사용자가 일기장에 직접 입력한 확정 데이터입니다. 값이 있으면 반드시 분석과 evidence에 인용하고 missingData에 같은 항목을 다시 요구하지 않습니다.
9-1. deviceMetrics는 Health Connect·스마트워치·헬스 앱에서 자동 수집된 관찰 데이터입니다. 값이 있으면 직접 입력값과 구분하여 분석하고, 걸음·수면·심박·운동·거리·칼로리 등 이미 존재하는 자동 수집 항목을 missingData로 반복 요청하지 않습니다.
10. 일기 본문에 운동 후 피로도·통증·체감 강도·발생 시각·지속시간이 적혀 있으면 해당 내용을 읽었다는 근거를 evidence에 남기고, 이미 적힌 정보를 missingData로 반복 요청하지 않습니다.
11. missingData는 dailyRecord와 extractedImageData 전체를 대조한 뒤 실제로 없는 정보만 반환합니다.
12. weather.available이 true이면 온도·습도·강수·바람을 운동 강도, 야외활동 안전, 수분 보충과 회복 추천에 반영하고 evidence에 날씨 근거를 남깁니다.
13. 더위·높은 습도·강한 바람·비·눈은 무리한 야외 운동을 권하지 않는 근거로 사용합니다. 날씨만으로 건강 상태를 단정하지 않습니다.
14. weather.available이 false이면 날씨를 추정하거나 날씨 기반 조언을 만들지 않습니다.
...
const requiredDailyInputRules = `

[일기장 직접 입력 검증 규칙 — 반드시 준수]
- dailyRecord.steps, sleepHours, waterLiters, narrative는 분석 버튼을 누르는 순간 화면에서 다시 읽은 값입니다.
- 값이 존재하는 걸음·수면시간·수분량은 evidence와 해당 분석 문단에 명시적으로 반영합니다.
- dailyRecord.deviceMetrics는 Health Connect 등 외부 기기에서 자동 수집된 값입니다. 값이 존재하면 직접 입력값과 구분하여 evidence와 분석 문단에 명시적으로 반영합니다.
- 이미 값이 있는 항목은 missingData에 다시 요구하지 않습니다.
- narrative에 운동 후 피로도, 체감 강도, 통증, 발생 시각, 지속시간이 있으면 이를 evidence에 인용하고 같은 정보를 다시 요구하지 않습니다.
- missingData는 dailyRecord와 extractedImageData에 실제로 존재하지 않는 정보만 반환합니다.
- weather.available이 true이면 날씨 수치를 evidence에 남기고, 운동·수분·야외활동 추천에 관련성이 있을 때 구체적으로 반영합니다.
- weather.available이 false이면 날씨 정보를 추정하지 않습니다.`;
const dailyAnalysisPrompt = () =>
  `${state.settings.customPrompt || defaultPrompt}${requiredDailyInputRules}`;
...
function hasUserContent(source = state) {
  ...
  const hasDay = Object.values(source.days || {}).some(day => day && (
    ...
    day.waterInput || day.analysis || day.extractedImageData || day.deviceMetrics || (day.attachments || []).length
  ));
...
function saveDay() {
  ...
  day.lastInputSnapshot = {
    narrativeLength: day.entryText.length,
...
function compactValue(value, unit = "") {
  ...
  if (typeof value === "number") return `${Number.isInteger(value) ? value.toLocaleString() : value.toFixed(value >= 10 ? 1 : 2).replace(/\.?0+$/,"")}${unit}`;
  return `${value}${unit}`;
}
function deviceMetricItems(metrics = {}) {
  ...
  return [
    ["걸음", compactValue(m.steps, "걸음"), "활동량"],
    ["거리", compactValue(m.distanceKm, "km"), "이동"],
    ["활동 칼로리", compactValue(m.activeCaloriesKcal, "kcal"), "운동"],
    ["총 칼로리", compactValue(m.totalCaloriesKcal, "kcal"), "일일"],
    ["활동 시간", compactValue(m.activeMinutes, "분"), "움직임"],
    ["운동", workouts.length ? `${workouts.length}회${workoutMinutes ? ` · ${workoutMinutes}분` : ""}` : "", "세션"],
    ["평균 심박", compactValue(m.heartRateAvgBpm, "bpm"), "심박"],
...
function renderDeviceMetrics() {
  const grid = $("#deviceMetricGrid");
  const status = $("#deviceSyncStatus");
  if (!grid) return;
  const metrics = getDay().deviceMetrics;
  const items = deviceMetricItems(metrics);
  if (!metrics || !items.length) {
    grid.innerHTML = `<div class="device-empty">Health Connect 연동 앱이 연결되면 걸음·수면·심박·운동 데이터가 여기에 자동 표시됩니다.</div>`;
    if (status) status.textContent = authSession ? "연동 대기" : "로그인 필요";
    return;
  }
  ...
  `).join("");
  if (status) {
    const source = metrics.source || metrics.provider || "Health Connect";
    const time = metrics.collectedAt || metrics.updatedAt;
    status.textContent = `${source}${time ? ` · ${new Date(time).toLocaleTimeString("ko-KR",{hour:"2-digit",minute:"2-digit"})}` : ""}`;
  }
}
async function loadDeviceMetricsForCurrentDate({silent = true} = {}) {
  if (!authSession) {
    renderDeviceMetrics();
    return;
  }
  try {
    const result = await apiFetch(`/api/device-data?date=${encodeURIComponent(dateKey())}`);
    const day = getDay();
    if (result?.data) {
      day.deviceMetrics = result.data;
      renderDeviceMetrics();
      persist();
    } else {
      delete day.deviceMetrics;
      renderDeviceMetrics();
    }
  } catch (error) {
    if (!silent) toast(error.message || "기기 데이터를 불러오지 못했어요");
    const status = $("#deviceSyncStatus");
    if (status) status.textContent = "연동 확인 실패";
  }
}
...
function parseKmaWeather(data) {
  ...
  return {
    temperatureC: values.T1H ?? values.TMP ?? null,
    humidityPercent: values.REH ?? null,
    windSpeedMs: values.WSD ?? null,
    precipitationMm: values.RN1 ?? null,
...
async function loadWeather() {
  ...
  try {
    ...
    $("#weatherLabel").textContent = `${location} · ${weather.description}${weather.temperatureC != null ? ` ${weather.temperatureC}°` : ""}`;
...
async function initAuth() {
  ...
  supabaseClient.auth.onAuthStateChange(async (event, session) => {
    ...
    await loadDeviceMetricsForCurrentDate();
    loadWeather();
  });
  await refreshAccount();
  await loadCloudSnapshot();
...
function shiftDay(amount) {
  ...
  loadDeviceMetricsForCurrentDate();
}
...
function recordedDays() {
  return Object.entries(state.days || {})
    .filter(([, day]) => day && (
      ...
      day.waterInput || day.analysis || day.deviceMetrics || (day.attachments || []).length
    ))
...
function buildPayload() {
  ...
  return {
    systemPrompt: dailyAnalysisPrompt(),
    healthProfileMarkdown: profileMarkdown(),
    healthProfileAttachments: volatileProfileAttachments.length ? volatileProfileAttachments : (state.profile.attachments || []),
    date: dateKey(),
    weather: {
      available:Boolean(day.weather?.available),
      location:day.weather?.location || state.settings.weatherLocation || "서울",
      description:day.weather?.description || null,
      temperatureC:day.weather?.temperatureC ?? null,
      humidityPercent:day.weather?.humidityPercent ?? null,
      windSpeedMs:day.weather?.windSpeedMs ?? null,
      precipitationMm:day.weather?.precipitationMm ?? null,
      precipitationType:day.weather?.precipitationType ?? null,
      observedAt:day.weather?.observedAt || null,
      displayText:$("#weatherLabel").textContent
    },
    dailyRecord: {
      ...
      sleepHours: day.sleepInput || null,
      waterLiters: day.waterInput || null,
      deviceMetrics: day.deviceMetrics || null,
      uploadedFiles: attachmentMetadata
    },
    inputVerification: {
      source: "일기장 1쪽 화면에서 분석 직전에 다시 수집",
      narrativeCharacters: (day.entryText || "").length,
      stepsEntered: Boolean(day.stepsInput),
      sleepEntered: Boolean(day.sleepInput),
      waterEntered: Boolean(day.waterInput),
      deviceMetricsAvailable: Boolean(day.deviceMetrics)
    },
...
function narrativeHas(text, pattern) {
  return pattern.test(String(text || "").replace(/\s+/g, " "));
}
function filterAlreadyProvidedMissingData(items = [], payload = buildPayload()) {
  const record = payload.dailyRecord || {};
  const narrative = record.narrative || "";
  const extracted = JSON.stringify(payload.extractedImageData || {});
  const device = record.deviceMetrics || {};
  const deviceText = JSON.stringify(device);
  const combined = `${narrative} ${extracted}`;
  const hasFatigueTracking = narrativeHas(narrative, /운동\s*후.{0,20}(피로|피곤|체감|강도|회복)|피로도|피로\s*[0-9]|RPE|체감\s*강도|지속\s*시간|지속시간/i);
  const hasWorkoutDetail = narrativeHas(`${combined} ${deviceText}`, /근력|웨이트|세트|횟수|회|분|시간|km|심박|bpm|페이스|케이던스|RPE|체감\s*강도|durationMinutes|heartRate|distanceKm|workouts/i);
  ...
  return (Array.isArray(items) ? items : []).filter(item => {
    ...
    if (/거리|활동 칼로리|운동/.test(deviceItems) && /운동\s*(종목|시간|거리|심박|강도)|활동량|칼로리|페이스|속도/.test(text)) return false;
    if (hasFatigueTracking && /운동\s*후.*피로|피로도|회복\s*체감|RPE|체감\s*강도/.test(text)) return false;
    if (hasWorkoutDetail && /운동\s*(종목|시간|거리|심박|강도)|근력운동.*(빈도|자세|기록)/.test(text)) return false;
    return true;
  });
}
function verifiedInputEvidence() {
  const day = getDay();
  const evidence = [];
  if (day.stepsInput) evidence.push(`일기장 직접 입력 걸음: ${day.stepsInput}걸음`);
  if (day.sleepInput) evidence.push(`일기장 직접 입력 수면: ${day.sleepInput}시간`);
  if (day.waterInput) evidence.push(`일기장 직접 입력 수분: ${day.waterInput}L`);
  if (day.deviceMetrics) {
    const items = deviceMetricItems(day.deviceMetrics).slice(0, 8).map(([label, value]) => `${label} ${value}`).join(", ");
    if (items) evidence.push(`스마트 기기 자동 수집 데이터: ${items}`);
  }
  if (day.entryText) evidence.push(`일기장 본문 ${day.entryText.length}자를 분석 입력으로 전달함`);
  if (day.weather?.available) {
    const weather = day.weather;
    const values = [
      weather.temperatureC != null ? `${weather.temperatureC}℃` : "",
      weather.humidityPercent != null ? `습도 ${weather.humidityPercent}%` : "",
      weather.windSpeedMs != null ? `바람 ${weather.windSpeedMs}m/s` : "",
      weather.precipitationMm != null ? `강수 ${weather.precipitationMm}` : ""
    ].filter(Boolean).join(" · ");
    evidence.push(`분석에 반영한 날씨: ${weather.location || ""} ${weather.description || ""}${values ? ` · ${values}` : ""}`.trim());
  }
  return evidence;
}
...
function renderReport(a) {
  ...
  const evidence = [...verifiedInputEvidence(), ...(a.evidence || [])]
    .filter((value, index, all) => value && all.indexOf(value) === index);
  $("#evidenceList").innerHTML = evidence.map(x => `<li>${escapeHtml(x)}</li>`).join("") || "<li>분석 근거가 제공되지 않았습니다.</li>";
  renderMissingData(filterAlreadyProvidedMissingData(a.missingData || []));
...
async function analyze() {
  ...
  try {
    ...
    if (authSession) {
      btn.textContent = "스마트 기기 데이터를 확인하는 중…";
      await loadDeviceMetricsForCurrentDate();
      saveDay();
    }
    ...
    analysis.missingData = filterAlreadyProvidedMissingData(analysis.missingData || [], payload);
    analysis.evidence = [...verifiedInputEvidence(), ...(analysis.evidence || [])]
      .filter((value, index, all) => value && all.indexOf(value) === index);
